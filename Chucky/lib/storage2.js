...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./lib/sampah/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./lib/sampah/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./lib/sampah/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./lib/sampah/${filename}.mp4`)
	Promise.all([unlink(`./lib/sampah/${filename}.mp4`), unlink(`./lib/sampah/${filename}.gif`)])
	return buffer5
				   }